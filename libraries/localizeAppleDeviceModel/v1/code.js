...
const iPhone = {
  ...
  const AppleWatch = {
    ...
    "Watch6,10": "Apple Watch SE 2 40mm ",
    "Watch6,11": "Apple Watch SE 2 44mm",
  ...
  export function getDeviceModel(platform) {
    if (iPhone[platform]) return iPhone[platform];
    if (iPod[platform]) return iPod[platform];
    if (iPad[platform]) return iPad[platform];
    if (iPadAir[platform]) return iPadAir[platform];
    if (iPadPro[platform]) return iPadPro[platform];
    if (iPadMini[platform]) return iPadMini[platform];
    if (AppleWatch[platform]) return AppleWatch[platform];
    if (Others[platform]) return Others[platform];
    return platform;
  }
  
  export function cleanDeviceModel(event) {
  let currentDeviceModel = event?.context?.device?.model;
    if (typeof currentDeviceModel !== "undefined") {
      event.context.device.model = getDeviceModel(currentDeviceModel);
    }
    return event;
  }
